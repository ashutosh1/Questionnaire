module GenerateAndSendSets
  ...
  def generate_and_send_sets(num_of_sets)
    ...
    send_data(File.new(Rails.root.join("public/reports/#{@test_set.file_name}.zip")).read, :type=>"application/zip" , :disposition => "attachment", :filename => "#{@test_set.file_name}.zip")
