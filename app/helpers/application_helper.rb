module ApplicationHelper
  def mark_required(object, attribute)
    "*" if object.class.validators_on(attribute).map(&:class).include? ActiveRecord::Validations::PresenceValidator
  end

  def display_nested_categories( node ) 
    html = "<li class='treeCategory' data-nodeid='#{node.id}'>"
    node_class = node.children.length == 0 ? "file" : "folder"
    html << "<span class=\"#{node_class}\">#{(node.name)}_#{node.id} </span>"
    html << "<span class='categoryDestroyLink'>#{link_to 'Destroy', category_path(node), method: :delete} </span>"
    html << "<ul class='cat_ul_drop' id=\"children_of_#{(node.id)}\" data-nodeid='#{node.id}'>"
    node.children.each{|child_node|
      html << display_nested_categories( child_node )
    }
    html << "</ul></li>"
    html.html_safe
  end




end
